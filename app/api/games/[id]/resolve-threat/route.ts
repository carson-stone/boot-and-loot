...
import { resolveThreat } from "@/lib/game/combat";
...
export async function POST(
  ...
  try {
    ...
    const { playerId, gameCardId, resolutionOptionId } = body;

    if (!playerId || !gameCardId || !resolutionOptionId) {
      return NextResponse.json(
        { error: "playerId, gameCardId, and resolutionOptionId are required" },
        { status: 400 },
      );
    }
    ...
    const result = await resolveThreat(gameId, playerId, gameCardId, resolutionOptionId, turnState);
